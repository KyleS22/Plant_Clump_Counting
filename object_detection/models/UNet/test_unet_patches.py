...
from utils.utils import get_unet_boxes, normalize
from utils.utils import compute_jaccard as compute_mask_jaccard
from utils.utils import compute_dice as compute_mask_dice
import utils.bbox

# ***** METHODS START HERE ***** #

def get_boxes_from_unet(boxes):
    '''
    Args:
...
def main(args):
    ...
    net_h = args.net_height # config model.net_input_height
    net_w = args.net_width  # config model.net_input_width
    nms_thresh = 0.45
    ...
    for patchfoldername in patchfoldernames:
        ...
        for filename in infiles:
            ...
            ## *** ACTUAL DETECTION *** ##
            boxes = get_unet_boxes(infer_model, [image_np], net_h, net_w, nms_thresh)[0]
            ...
            boxesDL = get_boxes_from_unet(boxes)
    ...
    argsString="Input Folder: {}\nOutput Folder: {}\nGround-truth Folder: {}\nMin Area Percent (alpha): {}\nMin Jaccard (IoU): {}\n".format(infolder,outfolder,xmlfolder,BBOX_ALPHA,IOU_THRESH_MIN)
    print(argsString)
...
if __name__ == '__main__':
    ## *** INPUT ARGS *** ##
    parser = argparse.ArgumentParser(description='Process a series of images using the RielCZ Unet API Implementation (*.h5).')
    ...
    parser.add_argument('-W', '--net-width', dest='net_width',
                        type=int, default=416,
                        help='the width of the input to the network (default is 416)')
    parser.add_argument('-H', '--net-height', dest='net_height',
                        type=int, default=416,
                        help='the height of the input to the network (default is 416)')
