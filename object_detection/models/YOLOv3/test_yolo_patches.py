...
FOR THE STRAWBERRY DETEFCTION PROJECT FOR CMPT 819,
AND CODE WRITEN BY EXPERIENCOR FOR YOLOV3 (PREDICT.PY)
'''
...
from utils.utils import get_yolo_boxes, normalize
import utils.bbox

# ***** METHODS START HERE ***** #

def get_boxes_from_YOLO(boxes, min_score_thresh=0.5):
    '''
    Args:
    ...
    for bbox in boxes:
        ...
        if score > min_score_thresh: # Pass confidence check
            tempDict = {}
            tempDict["ymin"] = bbox.ymin
            tempDict["xmin"] = bbox.xmin
            tempDict["ymax"] = bbox.ymax
            tempDict["xmax"] = bbox.xmax
            tempDict["class"] = bbox.get_label()
            tempDict["score"] = score * 100.
...
def correctBoxWidth(boxes, correction):
    '''
    Add a fixed value to the width in each box dictionary in a list of boxes
    
    :param list boxes: a list of box dictionaries to correct
    :param int correction: the constant value to add to the box widths
    :return: a list of box dictionaries with corrected widths
    :rtype: list
    '''
    toReturn = []
    for box in boxes:
        toReturn.append(returnBoxDict(box["xmin"]+correction, box["ymin"], box["xmax"]+correction, box["ymax"]))
    return toReturn
...
def main(args):
    ...
    ### DEFAULT YOLOV3 PARAMATERS ###
    net_h, net_w = 416, 416 # a multiple of 32, the smaller the faster
    nms_thresh = 0.45

    ### INPUT ARGS PARAMATERS ###   
    ...
    BBOX_CONFIDENCE = args.bbox_confidence # 0.5 default
    IOU_THRESH_MIN = args.iou_thresh_min
    
    infolder = args.input_folder[0]
    
    patchfoldernames = [f.name for f in os.scandir(infolder) if f.is_dir()]
    ...
    if not os.path.exists(outfolder):
        ...
        print("WARNING: Creating output directory: {}".format(outfolder))
    ...
    ### LOAD THE YOLOV3 WEIGHTS INTO MEMORY ###
    os.environ['CUDA_VISIBLE_DEVICES'] = "0" # TODO: fix args.device
    infer_model = load_model(PATH_TO_CKPT) 
    
    ## *** PREPARE TO BATCH THE PATCH *** ##
    for patchfoldername in patchfoldernames:
    
        patchfolder = os.path.join(infolder, patchfoldername)

        ## *** FETCH INPUT IMAGES *** ##
        infiles = []
        for f in os.listdir(patchfolder):
            temppath = os.path.join(patchfolder, f)
            if os.path.isfile(temppath):
                infiles.append(temppath)
        
        infiles.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0][os.path.splitext(os.path.basename(x))[0].rfind(args.delimiter)+1:]))
        
        runningBoxes = []
        runningWidth = 0

        output_folder = os.path.join(outfolder, patchfoldername) 
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)
        output_folder_orig = output_folder # Just in case it gets changed by save-in-new-folder
        ...
        for filename in infiles:
            ...
            if xmlfolder:
                xmlfilename = os.path.join(xmlfolder, patchfoldername, filenameBasename + ".xml")
            ...
            image_np = frame.copy() # So as not to override original image 

            ## *** ACTUAL DETECTION *** ##
            boxes = get_yolo_boxes(infer_model, [image_np], net_h, net_w, args.anchors, BBOX_CONFIDENCE, nms_thresh)[0]
            ...
            boxesDL = get_boxes_from_YOLO(boxes, min_score_thresh=BBOX_CONFIDENCE)
            ...
            runningBoxes.extend(correctBoxWidth(boxesDL, runningWidth))
            runningWidth += width # Save the current width, important for bounding box exports
            print(runningWidth) # DEBUG
        ...
        if not args.dry_run and args.save_aggregate_statistics:
            #### Found boxes as CSV ####
            with open(os.path.join(output_folder_orig, '{}_boxes.csv'.format(patchfoldername)), 'w+') as f:
                f.write("xmin,ymin,xmax,ymax\n")
                for box in runningBoxes:
    ...
    argsString="Input Folder: {}\nOutput Folder: {}\nGround-truth Folder: {}\nMin Area Percent (alpha): {}\nMin Confidence: {}\nMin Jaccard (IoU): {}\n".format(infolder,outfolder,xmlfolder,BBOX_ALPHA,BBOX_CONFIDENCE,IOU_THRESH_MIN)
    print(argsString)
...
if __name__ == '__main__':
    ## *** INPUT ARGS *** ##
    parser = argparse.ArgumentParser(description='Process a series of images using the Experiencor Keras-YoloV3 API Implementation (*.h5).')
    parser.add_argument('ckpt', metavar='C', nargs=1,
                        help='the path/filename to the trained weights (an H5 file)')
    parser.add_argument('anchors', metavar='R', nargs='+', type=int,
                        help='the YOLO anchors in the initial training config file')
    parser.add_argument('--class-name', type=str,
                        default='default',
                        help='the name of the class, if one is not outputted by the model (\'default\' is default)')
    parser.add_argument('-i', '--infolder', nargs=1, dest='input_folder',
                        default='.',
                        help='the input folder to patch subdirectories (current folder is default)')
    ...
    parser.add_argument('-D', '--device', type=str,
                        default='"0"',
                        help='the device on which to run the model ("0", GPU 0, is default)')
    parser.add_argument('-d', '--delimiter', type=str,
                        default='_',
                        help='the delimiter in the filename past which is the patch number of the original image; assumes patch number is the last part of the filename (\'_\', is default)')
