...
def create_model(
    ...
    if os.path.exists(saved_weights_name): 
        ...
        tb = sys.exc_info()[2]
        raise ValueError("Pretrained weights at '{}' not found!".format(saved_weights_name)).with_traceback(tb)
...
def _main_(args):
    ...
    if not args.dry_run:
        ...
        outfolder = None
    average_precisions = evaluate(infer_model, valid_generator, net_h=config['model']['net_input_height'], net_w=config['model']['net_input_width'], save_path=outfolder, save_format=config['model']['mask_save_format'])
    ...
    thresholds = []
    erodeKernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5,5)) # TODO: Try iteratively adjusting later
    ...
    for i in range(valid_generator.size()):
        ...
        for img in raw_image:
            ...
            for input, output in zip(predict_image, prediction):
                ...
                _, contours, _ = cv2.findContours(opened.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                
                rects = []
                for cnt in contours:
                    x, y, w, h = cv2.boundingRect(cnt)
                    rects.append(returnBoxDict(x, y, x+w, y+h))
                
                marked = input.copy()
                for rect in rects:
                    cv2.rectangle(marked, (rect["xmin"],rect["ymin"]), (rect["xmax"],rect["ymax"]), (0,255,0), 3)
...
if __name__ == '__main__':
    argparser = argparse.ArgumentParser(description='Evaluate U-Net model on any dataset')
    argparser.add_argument('-c', '--conf', help='path to configuration file')   
    argparser.add_argument('-n', '--dry-run', action='store_true', help='don\'t save any output')
