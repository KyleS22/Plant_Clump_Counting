...
from utils.mask_preprocess import custom_mask_func as custom_mask_function
...
def create_model(
    ...
    train_model = unet().create_unet_model((net_input_height, net_input_width, 3), nb_class)
    ...
    '''else:
        train_model.load_weights("backend.h5", by_name=True) # Should ideally have come from "pretrained_weights" but that apparently doesn't work'''     

    optimizer = Adam(lr=lr, clipnorm=0.001)
    train_model.compile(loss=loss_funct, optimizer=optimizer)             
...
def _main_(args):
    ...
    train_generator = BatchGenerator(
        ...
        norm                = normalize,
        custom_mask_func    = custom_mask_function if config['train']['use_custom_mask_func'] else None
    )
    ...
    '''valid_generator = BatchGenerator(
        ...
        max_box_per_image   = max_box_per_image,
        batch_size          = config['train']['batch_size'], # Make own validate one??? 
        #min_net_size        = config['model']['min_input_size'],
        #max_net_size        = config['model']['max_input_size'],
        net_input_height    = config['model']['net_input_height'],
        net_input_width     = config['model']['net_input_width'],
...
if __name__ == '__main__':
    argparser = argparse.ArgumentParser(description='train U-Net model on any dataset')
